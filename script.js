...
// Ensure smooth operation in MIT App Inventor WebView
if (typeof window !== 'undefined') {
  // Disable default touch behaviors that interfere with WebView
  document.addEventListener('touchmove', function(e) {
    if (e.target.closest('.quiz-mobile-container, .question-wrap')) {
      // Allow natural scrolling in main content areas
      return true;
    }
  }, { passive: true });
...
function setupLanding() {
  document.getElementById('btnPractice').addEventListener('click', openPracticeSetup);
  document.getElementById('btnExam').addEventListener('click', openExamSetup);
...
// ═══════════════════════════════════════════════════════
// JSON LOADING — MIT App Inventor Compatible
// ═══════════════════════════════════════════════════════

/**
 * Load questions from JSON file
 * Works with both:
 * 1. Local files (file://) in MIT App Inventor
 * 2. Remote files (http://)
 * 3. Relative paths
 */
async function loadSubjectQuestions(key) {
  try {
    ...
    // Try multiple paths for MIT App Inventor compatibility
    const paths = [
      `file:///android_asset/${key}.json`,           // Asset path (MIT App Inventor)
      `/assets/${key}.json`,                          // Absolute path
      `${key}.json`,                                  // Relative path
      `./assets/${key}.json`,                         // Relative with folder
    ];
    ...
    if (!res || !res.ok) {
      throw new Error(`Could not find ${key}.json in any location`);
    }
    ...
    if (Array.isArray(raw)) {
      ...
      // object format: {"Mathematics": [...]} or {"data": [...]}
    ...
    const normalized = arr
      .filter(q => q && (q.text || q.question))
      .map(q => normaliseQuestion(q, key));
    
    if (normalized.length === 0) {
      console.warn(`⚠ No valid questions found in ${key}.json`);
    }
    
    return normalized;
  } catch (e) {
    console.error(`✗ Failed to load ${key}.json:`, e.message);
